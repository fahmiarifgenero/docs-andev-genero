  ...
  <FileTree.Folder name="resources" defaultOpen>
    <FileTree.Folder name="views" defaultOpen>
      <FileTree.Folder name="auth" defaultOpen>
        <FileTree.Folder name="passwords" defaultOpen>
...
<<<<<<< HEAD
Halaman Register didapatkan dari Laravel UI pada saat instalasi project menggunakan Auth Scaffolding `php artisan ui bootstrap --auth`. Dan untuk list user ada pada URL `/register`.

![Register Page](/auth/register-page.png)

## Halaman Login

Halaman Login didapatkan dari Laravel UI pada saat instalasi project menggunakan Auth Scaffolding `php artisan ui bootstrap --auth`. Dan untuk list user ada pada URL `/login`.

![Login Page](/auth/login-page.png)

Untuk user yang sudah berhasil masuk dapat cek pada URL `/users` atau `/registered`.
=======
...
>>>>>>> 98cfc75 (add detail on other page, modified page eksternal on development journey)
